...
# Load JSON data_old
with open("data_old/sudan.json", "r") as f:
    json_data = json.load(f)

# Filter records after October 1, 2023
start_date = datetime(2023, 4, 4, tzinfo=timezone.utc)
end_date = datetime(2024, 9, 12, tzinfo=timezone.utc)

# Filtering records that are after the start_date
filtered_records = [record for record in json_data if end_date > datetime.fromisoformat(record['publishedAt'].replace('Z', '+00:00')) > start_date]
